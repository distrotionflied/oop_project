...
class menu_frame extends JFrame {
    ...
    ImageIcon tips = new ImageIcon("assets\\all.png");

    //
    String[] menu_namebtn = {"SINGLE PLAYER", "HOST GAME", "JOIN GAME", "SETTING", "QUIT"};

    JButton[] menu_btn = new JButton[menu_namebtn.length];
    ...
    public menu_frame() {
        ...
        menu_btn[3].addActionListener(Mysetting -> {
        ...
        // Online
                // ปุ่ม HOST GAME
        menu_btn[1].addActionListener(host -> {
            ServerHost.main(new String[]{});
            dispose();
        });

        // ปุ่ม JOIN GAME  
        menu_btn[2].addActionListener(join -> {
        ...
        // QUIT
        menu_btn[4].addActionListener(close -> {
            System.exit(0);
        });
    }
}
